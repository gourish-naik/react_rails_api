  ...
  endpoints: (builder) => ({
    ...
    getTodos: builder.query<TodosResponse, TodosQueryParams >({
      query: (params = {}) => {
        ...
        return `todos${searchParams.toString() ? '?' + searchParams.toString() : ''}`
      },
      providesTags: ['Todo'],
    }),
    ...
    // Get completed todos
    getCompletedTodos: builder.query<TodosResponse, { page?: number; order?: 'asc' | 'desc' }>({
      query: (params = {}) => {
    ...
    getIncompleteTodos: builder.query<TodosResponse, { page?: number; order?: 'asc' | 'desc' }>({
      query: (params = {}) => {
